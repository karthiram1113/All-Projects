  ...
  useEffect(() => {
    ...
    const shopTypeApi = async (e) => {
      ...
      try {
        ...
        const responseData = await shopTypeCreate(
         shopType,
        status
        );
    ...
    setSubmit(true)
      ...
      try {    
        ...
        const responseData = await shopEditFunction(
          shopTypeId,
         shopType,
          status
        );
   ...
   const shoTypeGetMethod = async (shopId) => {
    try {
        ...
        const response = await shopTypeGet(shopId);
  ...
  <div className="form-group">
    ...
    <select value={status} onChange={(e)=>setStatus(e.target.value)} className="form-control" id="exampleInputGender" style={{ height: '44px', fontSize: '12px',borderColor:"red" }}>
      ...
      <option value="Inactive">Inactive</option>
            ...
            <button onClick={()=>Navigate("/Shoptype-list")} class="btn btn-light">Cancel</button>
       {urlName=="Create"? <button type="submit" onClick={shopTypeApi} class="btn btn-gradient-primary me-2">Submit</button>:
        <button type="submit" onClick={shopEdit} class="btn btn-gradient-primary me-2">Submit</button>}    
