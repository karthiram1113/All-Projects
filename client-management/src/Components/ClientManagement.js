...
function ClientManagement() {
  ...
  const clientList = async (page) => {
    ...
    const response = await fetch(Url.start + Url.clientList, {
      method: "POST",
      headers: {
  ...
  const clientDelete = async () => {
    let token = localStorage.getItem("token");
    const response = await fetch(Url.start + Url.clientDelete + delete_id, {
      method: "DELETE",
      headers: {
        "content-type": "application/json",
        Authorization: "Bearer " + token,
      ...
      <main id="main" className="main">
            ...
            <div className="col-md-12">
              <nav>
                <ol className="breadcrumb">
                  ...
                  <li className="breadcrumb-item active">ClientManagement</li>
                </ol>
              </nav>
            </div>
          </div>
        ...
        <section className="section profile crud-top">
          <div className="card right-join">
            <div ref={cardRef} className={`card-body ${isOverflowing ? "over" : ""}`}>
              <div className="">
                {loading ? (
                        ...
                        {list.map((list, ind) => (
                          <tr key={list.id}>
                            ...
                            <td className="clients">
                              <li>
                                <Link to={{ pathname: `/ClientView/${list.id}` }} state={list} className="eye">
                                  <i className="fa-solid fa-eye eye"></i>
                                </Link>
                              </li>
                              <li>
                                <Link to={{ pathname: `/Clientpage/Edit/${list.id}` }} className="edit" state={list}>
                              ...
                              <li>
                                <Link
                                  className="delete"
                                  onClick={() => { setId(list.id); setValue(list.client_name); }}
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                Are You Sure Want To Delete Client Name <span style={{ color: "red", overflow: "hidden", wordBreak: "break-all" }}>{value}?</span>
              </div>
