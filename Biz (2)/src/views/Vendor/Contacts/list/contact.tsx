...
function StoreContacts() {
  ...
  const handlePhoneChange = (e: { target: { value: string } }) => {
      ...
      VendorAPI.contactGroupContactListAPI(apiData)
         .then((responceData: any) => {
            if (responceData.apiStatus.code === '200') {
               ...
               setGroupId(responceData.result?.GroupData?.groupId);
               setLoading(false)
            } else {
               if (responceData.apiStatus.code == "404") {
                  setListgroupContact([]);
   ...
   const superAdminContactCreate = () => {
      ...
      // if (modalMode === "edit" && (!fName || !mobNumber || !groupName || !storeName)) {
      //    return;
      // }
   ...
   const handleExport = async (name:any) => {
      ...
      try {
         var response;
         if(name==="withData"){
            response = await VendorAPI.exportContact(groupId);
         }else if(!groupId){
           response = await VendorAPI.exportContact();
         }else{
            response = await VendorAPI.exportHeaderContact();
         }
         // Validate that the response is a Blob-compatible object or ArrayBuffer
                  ...
                  <nav aria-label="breadcrumb">
                     <ol className="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li
                           ...
                           aria-current="page"
                        >
                           {location.pathname==="/vendor/groupcontacts" && gpStatus ? gpName + " Group Contacts" : "Customer Details"}
                        </li>
                     </ol>
                     <h6 className="text-start font-weight-bolder mb-0">
                        {location.pathname==="/vendor/groupcontacts" && gpStatus ? `${gpName} Group Contact (${grouptotalRecords})`: "Customer Details"}
                     </h6>
                  </nav>
               </div>
               <div className="col-md-7 text-end dropdown">
                  {location.pathname==="/vendor/groupcontacts" && gpStatus ?
                     <button className="vendor-crt-btn" onClick={() => { navigate('/vendor/contacts/groups') }}>
                        <span><i className="fa-solid fa-chevron-left"></i> Back to Contact Group</span>
                     </button>
                     : ""}&nbsp;
                 ...
                 {location.pathname==="/vendor/groupcontacts" && listgroupContact.length !== 0 && (
                  <button className="vendor-crt-btn" data-bs-toggle="modal" data-bs-target="#vendorExport">
                     Export Contact
                  </button>
                  )}
                     ...
                     <div className="card mb-4">
                        <div className="card-body px-0 pt-0 pb-2">
                           <div className="table-responsive p-0">
                              {location.pathname==="/vendor/contacts" && listContact.length >=1 ?
                                       <>
                                       <div className='d-flex justify-content-between align-items-center show-entries-main w-100 mt-n1 pb-0'>
                                          ...
                                          <div className='d-flex show-entries-main mt-n1 px-1'>
                                             <span className='show-entries-cnt'>Show</span> 
                                                   <div className="dropdown mt-2">
                                                      <button className="btn show-entries-btn dropdown-toggle mx-2" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                         {recordsPerPage}
                                                      </button>
                                                         <ul className="dropdown-menu show-entries-dropdown">
                                 ...
                                 loading ? (
                                    ...
                                    <><table className="table align-items-center justify-content-center mb-0">
                                       ...
                                       <tbody className="text-start">
                                          {location.pathname==="/vendor/groupcontacts" && gpStatus === true ?
                        ...
                        <div className="col-md-6 login-input-group">
                           <div className="vendor-create-container dropdown" data-bs-toggle="dropdown" aria-expanded="false">
                              <input type="text" id="vendor-crt-input"
                                 // style={submit && (!storeName || storeName.trim() === "") ? { borderColor: "red" } : { borderColor: "" }}
                           ...
                           {/* {submit && (!storeName || storeName.trim() === "") ? (
