...
import { deleteVendor, vendortList } from '../service/apiserver';
...
function VendorList() {
  ...
  const fetchVendorList = async (pageNumber) => {
    setLoading(true);
    try {
      const responseData = await vendortList(pageNumber - 1, recordsPerPage);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        fetchVendorList(currentPage); // Refetch list after deletion
            ...
            <div className="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ul className="breadcrumb">
                  <button
                    onClick={() => navigate("/vendorformpage/Create")}
                      ...
                      <tbody>
                        {list.map((value, ind) => (
                          <tr key={value.id}>
                            ...
                            <td>
                              <img
                                src={value.avatar ? `${API_BASE_URL}${value.avatar}` : "trty"}
                                alt="Product"
                            ...
                            <td className="clients">
                              ...
                              <li>
                                <Link
                                  to={`/vendorformpage/Edit/${value.id}`}
                                  className="edit"
                                >
