...
function VendorLogin() {
  ...
  const handleVendorLogin = (e:any) => {
    ...
    LoginAPI.signInAPI(apiData)
      .then((responseData: any) => {
        if (responseData.apiStatus.code === '200') {
          setLoading(false)
          setSubmit(false);
          localStorage.setItem("userVendorName", responseData.responseData?.userDetail?.username);
          localStorage.setItem("vendorToken", responseData.responseData.token);
                  ...
                  <div className="card card-plain mt-8">
                    ...
                    <div className="card-body">
                      <form role="form">
                        <label>Email</label>
                        <div className="mb-3">
                          <input required autoComplete="off" onChange={(e) => { setEmail(e.target.value); setLoading(false); }} type="email"
                        ...
                        <div className="mb-3 sadmin-passwordInput">
                          <input required autoComplete="off" onChange={(e) => { setPassword(e.target.value); setLoading(false); }} type={showPassword ? 'text' : 'password'}
