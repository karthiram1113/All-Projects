...
function Createcampaign() {
   ...
   const handlecreateCampaign = () => {
      setSubmit(true);
      if (!campaignName || !groupName || !groupId) {
         return;
      }
      setLoading(true)
      const apiData = {
   ...
   const handleBodyTextSelect = (item: string, option: any) => {
      setSelectedBodyText((prevSelectedValues) => {
         const exists = prevSelectedValues.some((p) => p.itemNumber === item);
         if (exists) {
            return prevSelectedValues.map((prev) =>
               prev.itemNumber == item ? { ...prev, varTypeId: option.id, varTypeName: option.variable_name } : prev
            );
         } else {
            return [...prevSelectedValues, { varTypeName: option.variable_name, varTypeId: option.id, itemNumber: item }];
         }
      });
                                    ...
                                    <div className="col-md-6 text-start ">
                                             ...
                                             <div className="row">
                                                ...
                                                <div className="col-md-12 mt-2">
                                                   ...
                                                   <div className="vendor-create-container mt-3 dropdown" data-bs-toggle="dropdown" aria-expanded="false">
                                                      <input type="text" id="vendor-crt-input mb-3" onClick={handleVariableDrop}
                                                         value={header_name} readOnly className="vendor-crt-input" placeholder=" " required />
                                          ...
                                          <div className="row">
                                             {BodytextNumbers.map((item) =>
                                                <div className="col-md-6 mt-2">
                                                   ...
                                                   <div className="vendor-create-container mt-3 dropdown" data-bs-toggle="dropdown" aria-expanded="false">
                                                      <input type="text" id="vendor-crt-input mb-3" onClick={handleVariableDrop}
                                                         value={inputValues[item] || ''} readOnly className="vendor-crt-input" placeholder=" " required />
                                                      <label htmlFor="vendor-crt-input" className="vendor-crt-label">Choose or write your own</label>
                                                      <i
                                                         className={"dropdown-icon font-size-dash-arrow fa-solid fa-chevron-down"}
                                                      ></i>
                                                         ...
                                                         {variableDrop?.map((dropdownValue, id) => (
                                                            <li key={id}>
                                                               <a
                                                                  className="dropdown-item"
                                                                  href="#"
                                                                  onClick={() => { handleBodyTextSelect(item, dropdownValue); setVariableId(dropdownValue?.id); setVariable_name(dropdownValue?.variable_name) }}
                                                               >
                                                                  {dropdownValue?.variable_name}
                                                               </a>
                                                            </li>
                                       ...
                                       <div className="text-start campaign-template mt-4">
                                          ...
                                          <div className="vendor-create-container dropdown" data-bs-toggle="dropdown" aria-expanded="false">
                                             <input type="text" id="vendor-crt-input" 
                                                onClick={handleGetGroupDrop} value={groupName}
                                          ...
                                          <div className="text-start pt-3 pb-3 form-check form-switch ms-1 is-filled">
                                             <input className="form-check-input campaign-create-inputstatus" type="checkbox" checked={restrictLangCode} onClick={ChecklangCode} id="flexSwitchCheckDefault" />
