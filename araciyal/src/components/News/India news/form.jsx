...
function INDForm() {
  ...
  const saveButton = async (e) => {
    ...
    try {
      const responseData = await INDcreate(
        title,
        sub_title,
        description,
        news_category,
  ...
  useEffect(() => {
    if (
      ...
      editorInstanceRef.current.getData() !== description
    ) {
  ...
  const updateFunction = async (e) => {
    ...
    try {
      const responseData = await INDUpdate(
        news_id,
        title,
        sub_title,
        description,
  ...
  const getMethod = async (news_id) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}${API_URL.tamilnaduGet}${news_id}`,
        {
          method: "GET",
          headers: {
      ...
      <div className="card-body">
        <form className="forms-sample" onSubmit={saveButton}>
          ...
          <button
            ...
            onClick={() => navigate("/IndiaNews-list/4")}
          >
            Cancel
          </button>
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
