...
import { Sportscreate, SportsUpdate, Sub_cate_dropdown, Spec_dropdown } from '../../service/apiserver';
...
function SportsForm() {
  ...
  const [errors, setErrors] = useState({});
    ...
    const saveButton = async (e) => {
        ...
        try {
            const responseData = await Sportscreate(
                title,
                sub_title,
                description,
                news_category,
            ...
            if (responseData.apiStatus?.code === "200") {
                toast.success(responseData.apiStatus.message || "News created successfully!");
                Navigate("/sportsnews-list/5");
            } else {
                toast.error(responseData.apiStatus.message || "Failed to create the News.");
            }
        } catch (error) {
    ...
    const updateFunction = async (e) => {
        ...
        try {
            const responseData = await SportsUpdate(
                news_id,
                title,
                sub_title,
                description,
    ...
    const getMethod = async (news_id) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}${API_URL.tamilnaduGet}${news_id}`, {
                method: "GET",
                headers: {
            ...
            setCurrentStatus(data.publish_status == 0 ? "Inactive" : "" || data.publish_status == 1 ? "Active" : "")
        } catch (errors) {
            ...
            try {
                ...
                if (!multiTimeApiCall) {
                    if (responseData.apiStatus.code === "200") {
                        ...
                        console.log("bk", responseData.result.SubcategoryData);
        ...
        <div className="card">
            <div className="card-body">
                <form className="forms-sample" onSubmit={saveButton}>
                    ...
                    <button type="button" className="btn btn-danger" onClick={() => navigate("/sportsnews-list/5")}>
                        Cancel
                    </button>

                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

                    {value === "Edit" ? <button className="btn btn-primary" onClick={updateFunction}>update</button> : <button onClick={saveButton} className="btn btn-primary" >Create</button>}
                </form>
            </div>
