...
import { Spec_dropdown, Sub_cate_dropdown, TNcreate, TNUpdate ,TNget } from "../../../api/services/NewsApi";
...
function TnForm() {
  ...
  const saveButton = async (e) => {
    ...
    try {
      const responseData = await TNcreate(
        title,
        sub_title,
        description,
        news_category,
      ...
      console.log(responseData,"title date image");
      
      console.log("coming herwe");
      
      if (responseData.apiStatus?.code === "200") {
        toast.success(
          responseData.apiStatus.message || "News created successfully!"
        );
        Navigate("/tamilnaduNews-list/3");
      } else {
        toast.error(
  ...
  const updateFunction = async (e) => {
    ...
    try {
      const responseData = await LoginAPI.TNupdate(
        news_id,
        title,
        sub_title,
        description,
  ...
  const getMethod = async (news_id) => {
    const token = localStorage.getItem("token");
    console.log(token,"token for get by id");
    console.log(news_id,"for get by id");
    

    try {
      const response = await TNget(news_id);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const responseData = await LoginAPI.dropdown(1);
        console.log(responseData,"karthi");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const responseData = await LoginAPI.Spec_dropdown(0, 10);
      ...
      <div className="card-body">
        <form className="forms-sample" onSubmit={saveButton}>
          <div className="row">
            <div className="col-md-6">
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                 className={`form-control ${errors.news_reporter ? "border-danger" : ""}`}
                  id="cost"
                  onChange={(e) => setnews_reporter(e.target.value)}
                  value={news_reporter}
                  placeholder="Reporter Name"
                />
                 {errors.news_reporter && <span className="text-danger">{errors.news_reporter}</span>}
              </div>
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                 className={`form-control ${errors.video_link ? "border-danger" : ""}`}
                  id="cost"
                  onChange={(e) => setnews_video_link(e.target.value)}
                  value={video_link}
                  placeholder="Specification"
                />
                {errors.video_link && <span className="text-danger">{errors.video_link}</span>}
              </div>
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                 className={`form-control ${errors.tags ? "border-danger" : ""}`}
                  id="cost"
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                <input
                  type="text"
                 className={`form-control ${errors.sub_title ? "border-danger" : ""}`}
                  id="categoryname"
              ...
              <div className="form-group">
                ...
                <select
                 className={`form-control ${errors.news_sub_category ? "border-danger" : ""}`}
                  id="subCategory"
                ...
                {errors.news_sub_category && <span className="text-danger">{errors.news_sub_category}</span>}
              </div>
              ...
              <div className="form-group">
                ...
                {errors.specification && <span className="text-danger">{errors.specification}</span>}
              </div>
          ...
          <button
            ...
            onClick={() => navigate("/tamilnaduNews-list/3")}
          >
            Cancel
          </button>
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
