...
import TamilNadu, { TNdeleteProduct } from "../../../api/services/NewsApi";

function Tamilnadu_news() {
  ...
  const fetchDataList = async (pageNumber) => {
    ...
    try {
      const responseData = await TamilNadu.Tamilnadulist( 
        token,
        pageNumber - 1,
        recordsPerPage
      );
      console.log(responseData,"tamilnadu");
      ...
      if (responseData.apiStatus.code === "200") {
        ...
        localStorage.setItem(
          "SubcategoryId",
          responseData.result.newsData.news_sub_category_id
        );
        localStorage.setItem("TamilnaduNewsCount", totalRecords);
      } else {
  ...
  const handleDelete = async (news_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setLoading(true);
        try {
          const response = await TNdeleteProduct.Tamilnadudelete(news_id);
          if (response.success) {
            toast.success("News deleted successfully!");
            setList(list.filter((vendor) => vendor.news_id !== news_id));
          } else {
            ...
            <div className="page-header">
              ...
              <nav aria-label="breadcrumb">
                <ul className="breadcrumb">
                  <button
                    onClick={() => navigate("/tamilnaduNews/create")}
                            ...
                            <tr key={value.id}>
                              ...
                              <td className="text-start">
                                ...
                                {value.title.split(" ").length > 5 && (
                                  <span
                                    ...
                                    onClick={() =>
                                      toggleExpand(value.news_id, "title")
                                    }
                                  >
                                    {expandedItems[`${value.news_id}-title`]
                                      ? "Show less"
                                      : "Show more"}
                                  </span>
                                )}
                              ...
                              <td className="text-start">
                                ...
                                {value.sub_title.split(" ").length > 5 && (
                                  <span
                                    ...
                                    onClick={() =>
                                      toggleExpand(value.news_id, "sub_title")
                                    }
                                  >
                                    {expandedItems[`${value.news_id}-sub_title`]
                                      ? "Show less"
                                      : "Show more"}
                                  </span>
                                )}
                              ...
                              <td className="clients">
                                ...
                                <li>
                                  ...
                                  <Link
                                    to={`/Tamilnadu/Edit/${value.news_id}`}
                                    className="edit"
                                  >
