...
function GalleryVideo_Form() {
  ...
  const VideoCreate = async () => {
    ...
    try {
      const response = await LoginAPI.videoCreate(formData);
  ...
  const CaseStudyUpdate = async () => {
    ...
    const formData = new FormData();
    formData.append("video_id", getid);
    formData.append("title", title);
    formData.append("description", description);
    formData.append("video_url", videourl);
    ...
    try {
      const response = await LoginAPI.videoUpdate(formData);
              ...
              <div class="container-xxl flex-grow-1 container-p-y text-start">
                <h4 class="fw-bold py-3 mb-4">
                  <Link to={"/gallery-video"}>
                    <span class="text-muted fw-light">Gallery Image /</span>
                  </Link>{" "}
...
export default GalleryVideo_Form;
