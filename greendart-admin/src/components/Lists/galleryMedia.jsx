...
function GalleryMedia_List() {
  ...
  const Portfoliolist = async (page) => {
    setLoading(true);
    try {
      const responseData = await LoginAPI.mediaList({
        pageIndex: page - 1,
        dataLength: recordsPerPage,
      });
      if (responseData.apiStatus.code === "200") {
        setPortfolioList(responseData.responseData.MediaList);
        setGetId(responseData.responseData.MediaList.id);
  ...
  const handledelete = async () => {
    try {
      const response = await LoginAPI.mediaDeleteAPI(id);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      const response = await LoginAPI.mediaActive(id);
      if (response?.apiStatus?.code === "200") {
...
export default GalleryMedia_List;
