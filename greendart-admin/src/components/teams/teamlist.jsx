...
const TeamList = () => {
  ...
  const fetchList = async () => {
    ...
    try {
      const res = await LoginAPI.teamList(apiData);

      if (res.apiStatus.code === "200") {
        setData(res.responseData.teamList);
      } else {
  ...
  const confirmDelete = async () => {
    try {
      const res = await LoginAPI.teamDelete(deleteId);
  ...
  const handleStatusToggle = async (id) => {
    try {
      const res = await LoginAPI.teamStatus(id);
                    ...
                    >
                      <thead className="bg-light">
                        <tr>
                          ...
                          <th className="fw-bold text-center">Role</th>
                          ...
                          data.map((item, index) => (
                            <tr key={item.id}>
                              ...
                              <td className="text-center">
                                <img
                                  src={`${baseURL}${item.imgData.path}${item.imgData.altered_file_name}`}
                                  alt={item.heading}
                                  width={70}
                                  height={50}
                                  style={{
