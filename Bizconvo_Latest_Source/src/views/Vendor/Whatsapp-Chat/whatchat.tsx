...
console.log(slides, "slides in chat");
...
//     const firstCarouselMsg = chatList.find(
//       (msg) =>
...
//     if (firstCarouselMsg) {
//       const cards = firstCarouselMsg.messageBody.templateText.cards.map((card: any, idx: number) => {
//         const isVideo = card.header_image_link?.endsWith('.mp4');
//         return {
//           id: `${firstCarouselMsg.time}-${idx}`,
...
//   }
// }, [chatList]);
    const [carousels, setCarousels] = useState<{ [key: string]: any[] }>({});
    ...
    useEffect(() => {
        if (Array.isArray(chatList)) {
            ...
            carouselMessages.forEach((msg, msgIdx) => {
                mapped[msgIdx] = msg.messageBody.templateText.cards.map((card: any) => {
                    let link = card.header_image_link;
                    ...
                    return {
                        id: `${msg.time}-${card.card_index}`,
                        src: link,
                        format: isVideo ? "video" : "image",
                        title: card.body_texts,
            ...
            setCarousels(mapped); // ✅ each carousel separate
        }
    }, [chatList]);
                                                                ...
                                                                >
                                                                                                    {/* {slides.map((slide: any, index: number) => (
                                                                                                        <article key={index} style={{ width: `${100 / slides.length}%` }}>
                                                                                                            <div className={`info ${slide.position || ""}`}></div>
                                                                                                            {slide.format === 'video' ? (
                                                                                                                <video
                                                                                                                    key={slide.id}
                                                                                                                    src={slide.src}
                                                                                                    ...
                                                                                                    {Object.keys(carousels).map((cIndex) => (
                                                                                                        <div key={cIndex} className="carousel">
                                                                                                            {carousels[cIndex].map((slide: any, index: number) => (
                                                                                                                <article key={slide.id} style={{ width: `${100 / carousels[cIndex].length}%` }}>
                                                                                                                    <div className={`info ${slide.position || ""}`}></div>
                                                                                                                    {slide.format === "video" ? (
                                                                                                                        <video src={slide.src} controls autoPlay muted playsInline />
                                                                                                                    ) : (
