   ...
   return (
      ...
      {/* {BodytextNumbers1.length > 0 && ( */}
          ...
          <div className="row">
            {BodytextNumbers1.map((varName) => {
              ...
              const value = inputValues[inputKey] || (() => {
                // Initialize from carouselVariables state on first render
        ...
        {(() => {
         ...
         // Filter to ensure only one QUICK_REPLY is rendered
         const uniqueButtons = buttons.filter((btn: any, index: number, self: any[]) =>
            self.findIndex((b: any) => b.type === btn.type && b.text === btn.text) === index
         );

         return uniqueButtons.map((btn: any, idx: number) => {
            ...
            return (
               ...
               {/* Button Text Input (for all types) */}
             <div className={`${uniqueButtons.length===1? "col-md-12":"col-md-6"} login-input-group`}>
               ...
               <div className="vendor-create-container mt-3">
                 <input autoComplete="off"  type="text" id="vendor-crt-input" 
                  value={textValue}
                  onChange={(e) =>
                     handleInputChange(currentCarousel, `button-${idx}-text`, e.target.value)
                  }
                  className={`vendor-crt-input loginfilled-frame-username ${submit && !textValue ? 'error' : ''}`}
                  placeholder=" " maxLength={12} required />
                  <label htmlFor="vendor-crt-input" className="vendor-crt-label">Button Text</label>
               </div>
              {submit && !textValue && (
                     <div className="invalid-feedback d-block">Field is required</div>
                  )}
            </div>
               {/* URL Example Input (only for URL buttons) */}
               {/* {btn.type === "URL" && (
                  <div className="col-md-6 mt-2">
                     {`Assign URL param (example) for Button {{${idx + 1}}}`}
                     <div className="vendor-create-container mt-3">
                     <input
                        type="text"
                        id={`vendor-crt-button-example-${idx}`}
                        autoComplete="off"
                        value={exampleValue}
                        onChange={(e) =>
                           handleInputChange(currentCarousel, exampleVarName, e.target.value)
                        }
                        className={`vendor-crt-input loginfilled-frame-username ${submit && !exampleValue ? 'error' : ''}`}
                        required
                     />
                     <label htmlFor={`vendor-crt-button-example-${idx}`} className="vendor-crt-label">
